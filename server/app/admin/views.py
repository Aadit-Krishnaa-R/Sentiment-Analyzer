...
def convert_numpy_floats(data):
  """
  Converts NumPy floats in a dictionary to regular Python floats.

  Args:
      data: A dictionary where values may contain NumPy floats.

  Returns:
      A new dictionary with the same structure as the input, but with
      NumPy floats converted to regular Python floats.
  """
  if isinstance(data, dict):
    return {key: convert_numpy_floats(value) for key, value in data.items()}
  elif isinstance(data, list):
    return [convert_numpy_floats(item) for item in data]
  elif isinstance(data, np.float32):
    return float(data.item())  # Convert NumPy float to Python float
  else:
    return data
...
def top_employees_route():
    ...
    if admin:
        ...
        calls_today = 0
        today_start = time.mktime(datetime(year=2024, month=5, day=2).timetuple())
...
# graph_coords,emotions,pos_percent,neg_percent,rating,language,duration,transcript,issue,emotions_audio,employeename,created_at=None)
...
def predict():
    if request.method == 'POST':
        try:
            ...
            print(save_path)
            insertDict={}
            result = dHexagonAnalysis(save_path).__dict__
            print(type(result))
            username = request.form.get('username')
            result['employeename'] = username
            result['created_at']=time.time()
            converted_dict = convert_numpy_floats(result.copy())
            call_collection = db['calls']
            print("hhhjh",converted_dict)


            # insertDict=Call.to_dict(result.coordinates,result.emotions,result.pos_percent,result.neg_percent,result.rating,result.language,result.duration,result.transcript,result.issue,result.emotions_audio,result.employeename,result.created_at)
            call_collection.insert_one(converted_dict)
...
def voice_upload():
    try:
        ...
        result = dHexagonAnalysis(audio_path).__dict__
        print(type(result))
        # username = request.form.get('username')
        result['employeename'] = "test"
        result['created_at']=time.time()
        ...
        call_collection = db['calls']
        print("hhhjh",converted_dict)
        new_call=call_collection.insert_one(converted_dict)
        call_id=str(new_call.inserted_id)
