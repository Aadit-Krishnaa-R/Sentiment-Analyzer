...
class resultkoodhi:
    def __init__(self, graph_coords,emotions,pos_percent,neg_percent,rating,language,duration,transcript,issue_list,emotions_audio):
...
def classify_audio(audio_file_path):
    ...
    model = load_model("/home/aadit/Sentiment-Analyzer/model/recog_speech.h5")
    ...
    loaded_scaler = joblib.load('/home/aadit/Sentiment-Analyzer/model/scalerhybrid.pkl')
...
def issue_classify(text):
    ...
    genai.configure(api_key="AIzaSyANvcHmPabAo1dpaugcb7cSSW7104XbUHo")
    model = genai.GenerativeModel('gemini-pro')
    response = model.generate_content(
        f"Hey this is a customer call's transcript , can you categorise his issue into one category: {text}?")
...
def audio_to_text(audio_chunk):
    ...
    results = model.transcribe(audio_chunk,language="en")
    return results["text"]
...
def dHexagonAnalysis(audio_path):
    ...
    for j, audio_chunk in enumerate(audio_features):
        ...
        output_directory = "/home/aadit/Sentiment-Analyzer/model/"
        ...
        text_classification = classify_mood(text,emotions,emotion_weights)
    ...
    chunk.export(temp_filename, format="wav")
    text = audio_to_text(temp_filename)
    issue_first20 = issue_classify(text)
    os.remove(temp_filename)
    ...
    model_result = resultkoodhi(normalized_combined_results,
                          emotions,
                          pos_percentage,
                          neg_percentage,
                          rating_var,
...
def spam_detection(audio_path):
    ...
    #
    # else:
    results=dHexagonAnalysis(audio_path)
    return results

# result=spam_detection('sihaudio.wav')
# print(f"graph graph_coords->{result.graph_coords}")
